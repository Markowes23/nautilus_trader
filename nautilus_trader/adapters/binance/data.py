...
class BinanceCommonDataClient(LiveMarketDataClient):
    ...
    def __init__(
        ...
        # Delay for bulk subscriptions to come in
        self._connect_websockets_delay: float = 0.0
    ...
    async def _subscribe(self, command: SubscribeData) -> None:
        ...
        if instrument_id is None and command.data_type.type not in self._subscribe_allow_no_instrument_id:
            self._log.error(
    ...
    async def _unsubscribe_bars(self, command: UnsubscribeBars) -> None:
        ...
        if self._binance_account_type.is_futures and resolution == "s":
            self._log.error(
                f"Cannot unsubscribe from {command.bar_type}. Second interval bars are not aggregated by Binance Futures",
            )
        try:
            interval = BinanceKlineInterval(f"{command.bar_type.spec.step}{resolution}")
        except ValueError:
    ...
    async def _request_bars(self, request: RequestBars) -> None:
        ...
        if request.bar_type.is_externally_aggregated() or request.bar_type.spec.is_time_aggregated():
        ...
        if not bars:
            self._log.warning(
                f"No bars returned for {request.bar_type} between {request.start} and {request.end}",
            )
            return
